...
thread_unblock (struct thread *t)
{
  ...
  if (thread_current ()->priority < higher_pt->priority) {
    //if interrupt context --> int_yield on reutrn; else thread_yield
    if (intr_context ()) {
      intr_yield_on_return ();
    } else {
      // Only once the idle thread will run. Main thread will wait for the idle thread to start.
      // If we yield the idle thread, then the main thread will not be initialized and lead to deadlock
